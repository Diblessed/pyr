...
class Boss(sprite.Sprite):
    ...
    def logic(self,ast):
        if self.health == 30 and self.health == 25 and self.health == 20 and self.health == 15 and self.health == 10 and self.health == 5:
...
class Asteroid():
    def __init__(self, player_image,player_speed, dirx, diry, size_x, size_y, direction):
        super().__init__()
    ...
    def update(self):
        if self.direction == 0:
            self.rect.x += self.speed + randint(7, 10)
            self.rect.y += self.speed 
    ...
    def update3(self):
        collide = bullets.collidepoint(self)
        self.kill() if collide else print(1)
...
def boss_fight():
    ...
    player = Player('rocket.png', 15, 600, 690, 100, 110)
    asteroid = Asteroid('asteroid.png', 3,770, 0, 130, 130, randint(0,1))
    ...
    while fight:
        ...
        r_hand_boss.logic(asteroid)
        l_hand_boss.logic(asteroid)
        r_hand_boss.update()
        l_hand_boss.update()
